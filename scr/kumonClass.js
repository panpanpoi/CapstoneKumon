// =========================================================
// kumonClass.js — fetch/add/edit/display students
// =========================================================
...
window.initKumonClass = () => {
 ...
 addStudentBtn?.addEventListener("click", async (e) => {
  ...
  if (!level || !s1Day || !s1Start || !s1End) {
   alert("⚠️ Please fill in all required fields: Level and Schedule 1.");
   return;
  }
  ...
  try {
   ...
   if (s2Day && s2Start && s2End) {
    formData.append("schedule2_day", s2Day);
    formData.append("schedule2_start", s2Start);
    formData.append("schedule2_end", s2End);
   } else {
        // Explicitly send empty strings if not filled
        formData.append("schedule2_day", "");
    formData.append("schedule2_start", "");
    formData.append("schedule2_end", "");
      }
