...
require_once __DIR__ . '/auth.php'; // ensures logged-in
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (isset($_POST['book'], $_POST['schedule_id'])) {
            ...
            // ✅ FIX: Check for active bookings ONLY in the FUTURE or TODAY.
            // We join with ptc_schedules to check the date/time.
            // This ignores your past (Nov 15) booking so you can book again.
            $stmt = $pdo->prepare("
                SELECT COUNT(*) 
            ...
            if ($stmt->fetchColumn() > 0) {
                throw new Exception("You already have an upcoming active booking.");
            }

            // Validate the schedule we are trying to book
            $stmt = $pdo->prepare("
            ...
            // Double check: Prevent booking past slots
            $scheduleTime = new DateTime($schedule['date'] . ' ' . $schedule['startTime']);
            if ($scheduleTime < new DateTime()) {
                throw new Exception("Cannot book a past schedule.");
            }
...
// A. Fetch Current Booking (Future Only)
// This matches the logic used above, ensuring consistency.
$currentBooking = null;
$stmt = $pdo->prepare("
